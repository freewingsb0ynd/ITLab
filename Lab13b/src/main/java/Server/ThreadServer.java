...
public class ThreadServer extends Thread{
        ...
        while(run){
            ...
            if(nickName.compareTo("0")==0){
                ...
                if(checkNick(nickName)){
                    ...
                    server.user.append(nickName+" connected to chatroom\n");
                    server.sendAll(nickName,nickName+" joined chatroom\n");
    ...
    private void exit(){
        try {
            ...
            dis.close();
            client.close();
            server.user.append(nickName+" exitted\n");
            server.sendAll(nickName,nickName+" exitted\n");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
